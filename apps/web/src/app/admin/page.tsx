...
export default async function AdminPage() {
  ...
  return <main className="board-shell"><BoardHeader />
    ...
    <section className="admin-grid">
      <article className="admin-card"><p className="section-kicker">Campus status</p><h2>Loyola Academy</h2>{loyola?.status === "approved" ? <p><strong className="admin-live">Live</strong> Available in the college picker for new members.</p> : <><p>Make Loyola Academy available in the college picker.</p><form action={addLoyolaAcademy}><button className="primary-button" type="submit">Approve Loyola Academy</button></form></>}</article>
