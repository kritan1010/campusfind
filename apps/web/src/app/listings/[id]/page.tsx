...
export default async function ListingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="board-shell"><BoardHeader /><article className={`case-file ${listing.kind}`}>
    <header className="case-header"><div><p className="eyebrow">Case file · {listing.id.slice(0, 8)}</p><div className="evidence-row"><span className={`kind-label ${listing.kind}`}>{listing.kind}</span><span>{listing.status.replaceAll("_", " ")}</span></div><h1>{listing.title}</h1><p>Reported by {posterResult.data?.display_name ?? "a CampusFind member"}</p></div>{isPoster ? <div className="owner-actions"><Link className="secondary-button" href={`/listings/${id}/edit`}>Edit report</Link><Link className="secondary-button" href={`/listings/${id}/matches`}>View matches</Link>{listing.kind === "found" && <Link className="secondary-button" href={`/listings/${id}/claims`}>Review claims</Link>}{listing.status === "open" && <form action={closeListing}><input type="hidden" name="listingId" value={id} /><button className="secondary-button" type="submit">Close listing</button></form>}<form action={deleteListing}><input type="hidden" name="listingId" value={id} /><button className="danger-button" type="submit">Delete</button></form></div> : listing.kind === "found" && listing.status === "open" ? <Link className="primary-button" href={`/listings/${id}/claim`}>This is mine</Link> : null}</header>
