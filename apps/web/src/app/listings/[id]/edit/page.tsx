...
export default async function EditListingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [listingResult, zonesResult, imagesResult, attributesResult, locationResult] = await Promise.all([
    supabase.from("listings_public").select("id, poster_id, kind, title, description, category, colour, brand, model, event_date, zone_id").eq("id", id).eq("poster_id", user.id).maybeSingle(),
